...
describe('vue component library', () => {
   test('should output tree shakeable css module code', async () => {
      ...
      await build({
         logLevel: 'silent',
         configFile: path.resolve(__dirname, '../kolibry.config.lib.ts'),
      })
      // Build app
      const { output } = (await build({
         logLevel: 'silent',
         configFile: path.resolve(__dirname, '../kolibry.config.consumer.ts'),
      })) as RollupOutput
   ...
   test('should inject css when cssCodeSplit = true', async () => {
      ...
      const { output } = (
         await build({
            logLevel: 'silent',
            configFile: path.resolve(__dirname, '../kolibry.config.lib-css.ts'),
         })
      )[0]
