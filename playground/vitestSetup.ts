...
export const isBuild = !!process.env.KOLIBRY_TEST_BUILD
...
let server: KolibryDevServer | http.Server

/**
 * Kolibry Dev Server when testing serve
 */
export let kolibryServer: KolibryDevServer
/**
 * Root of the Kolibry fixture
 */
...
/**
 * current test using kolibry inline config
 * when using server.js is not possible to get the config
 */
export let kolibryConfig: InlineConfig | undefined
...
export let kolibryTestUrl = ''
export let watcher: RollupWatcher | undefined

declare module 'kolibry' {
...
export function setKolibryUrl(url: string): void {
   kolibryTestUrl = url
}
...
beforeAll(async (s) => {
   ...
   globalConsole.warn = (msg, ...args) => {
      ...
      if (msg.includes('@vue/reactivity-transform'))
         return
   ...
   try {
      ...
      // ignore favicon request in headed browser
         if (
            process.env.KOLIBRY_DEBUG_SERVE
      ...
      // start a kolibry server in that directory.
      if (testName) {
         testDir = resolve(workspaceRoot, 'playground-temp', testName)

         // when `root` dir is present, use it as kolibry's root
         ...
         if (testCustomServe) {
            ...
            if (serve) {
               server = await serve()
               kolibryServer = mod.kolibryServer
               return
            }
         }
         else {
...
export async function startDefaultServe(): Promise<void> {
   ...
   if (!isBuild) {
      process.env.KOLIBRY_INLINE = 'inline-serve'
      const testConfig = mergeConfig(options, config || {})
      kolibryConfig = testConfig
      kolibryServer = server = await (await createServer(testConfig)).listen()
      // use resolved port/base from server
      const devBase = server.config.base
      kolibryTestUrl = `http://localhost:${server.config.server.port}${
      devBase === '/' ? '' : devBase
    }`
      await page.goto(kolibryTestUrl)
   }
   else {
      process.env.KOLIBRY_INLINE = 'inline-build'
      // determine build watch
      const resolvedPlugin: () => PluginOption = () => ({
         name: 'kolibry-plugin-watcher',
      ...
      kolibryTestUrl = previewServer.resolvedUrls.local[0]
      await page.goto(kolibryTestUrl)
   }
}
